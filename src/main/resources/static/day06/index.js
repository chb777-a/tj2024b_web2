...
function fun1(param1 , param2){}
    // 정의2 : 익명 함수, 변수/상수 저장해서 사용한다. , 이벤트 함수
const fun2 = function ( param1 , param2){}
    // 정의3 : 화살표/람다식 함수, 주로 변수/상수 저장해서 사용한다. , 이벤트 함수
...
// [3] 객체 : {} 객체 : key와 value으로 한쌍을 구성하여 여러쌍을 {} 묶음 , [] 리스트/배열
...
const user = { name5 : '신동엽' , age5 : 57}
const {name5 , age5} = user; // 객체 내 동일한 속성명으로 변수를 여러개 분해해서 저장할 수 있다
console.log(name5)
console.log(age5)

// [6] 비구조화 할당과 나머지 연산자 , 배열 또는 객체 뒤에 ...이용하여 나머지는 객체 또는 배열로 저장하는 방법
const [array6, ...rest] = [1 , 2 , 3 , 4 , 5]
console.log(array6) // 1
console.log(rest) // 나머지.

const{name6 , ...rest2} = {name6 : '안녕하시오' , age : 70 , phone : '010'}
console.log( name6 ) // 안녕하시오
...
console.log( loginState == true ? '로그인상태' : '비로그인상태')
    // 2. 단축 연산/평가
        ...
        // 조건(A) || 거짓(B) : 만약에 조건 A가 true 이면 A 반환 false 이면 B 반환
...
// [8] 동기 = 통신요청 보내고 응답이 올 때까지 대기상태 , 비동기 = 통신요청 보내고 응답이 오기전에 다른 코드 처리
...
const arr9 = [1 , 2 , 3 , 4 , 5]
    ...
    //2. 향상된 for 문 , in : 인덱스 순회 , of : 값 순회
        // 반복변수명 in 배열명 : 인덱스 순회
        // 반복변수명 of 배열명 : 값 순회
    for (let i in arr9){console.log(i);}
    for (let v of arr9){console.log(v);}
    //3. forEach 문 , 배열명.forEach((반복변수명,인덱스변수명)=>{})
arr9.forEach((value ,index) => {console.log(value,index)});
    //4. map 문 : forEach함수는 return이 없고 map은 return이 가능하다
const array10 = arr9.map((value,index) => {console.log(value,index); return value*10;})
console.log(array10)
    //5. filter : 조거넹 맞는것만 turn 할 수 있다.
const array11 = arr9.filter((value,index)=>{return value%4 === 0 }) // 4배수
console.log(array11)