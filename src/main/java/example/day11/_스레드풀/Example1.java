...
public class Example1 {
    ...
    // main 함수 안에는 main 스레드(코드를 읽어오는 역할) 내장
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 5 ; i++){
            ...
            // (2) 작업 스레드 실행
            thread.start(); // 여러 작업 스레드간의 실행순서는 보장이 안 된다.
        } // for end
        ...
        for (int i = 1; i<=10; i++) {
            String name = "thread" + i;
            // (3) 새로운 스레드가 아닌 스레드풀에 정의한 스레드를 등록한다. .submit(등록할 스레드 인터페이스)
            // executorService.submit(등록할 스레드인터페이스)
