...
public class Example1 { // 클래스
    ...
    public static void main(String[] args) {
        ...
        // 3. 추상메소드를 람다 표현식으로 호출 -> , JS(화살표함수비슷 =>)
        Calculator cal3 = (x , y) -> x + y;
        int result3 = cal3.plus(3 , 5);
        System.out.println(result3);

        // 4. 람다표현식 제공하는 타입 , Function< 매개변수타입 , 리턴값타입 > , .apply( 인자값 )
            // Function<매개변수타입 , 리턴값타입 > , .apply(인자값)
            Function< Integer , Integer > fucntion = x -> x * 2;
            System.out.println(fucntion.apply(3)); // 3 --> x : 3 * 2
            System.out.println(fucntion.apply(7)); // 7 --> x : 7 * 2
